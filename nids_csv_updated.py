...
f=open(path)
data_Validate=pd.read_csv(f, header=None)

# Check if first row is a header (contains non-numeric)
if not pd.to_numeric(data_Validate.iloc[0, 0], errors='coerce') == data_Validate.iloc[0, 0]:
    # It's a header, reload skipping first row or just drop it
    data_Validate = data_Validate.iloc[1:]

# Clean infinite values and NaNs
# Clean infinite values and NaNs
data_Validate = data_Validate.replace(['Infinity', 'infinity'], np.nan)
data_Validate = data_Validate.apply(pd.to_numeric, errors='coerce')
data_Validate = data_Validate.replace([np.inf, -np.inf], np.nan).fillna(0)

data_Validate=data_Validate.iloc[:, :16]
...
data_Validate['protocol_type'] = protocol_type_le.fit_transform(data_Validate['protocol_type'].astype(str))
data_Validate['service'] = service_le.fit_transform(data_Validate['service'].astype(str))
data_Validate['flag'] = flag_le.fit_transform(data_Validate['flag'].astype(str))
